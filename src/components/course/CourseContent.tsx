                ...
                <AccordionContent className="pt-4 pb-0">
                  <div className="space-y-3 pl-4 border-l-2 border-gray-200 ml-6">
                    {chapter.sessions.map((session, sessionIndex) => {
                      ...
                      let hasRegisteredSlot = false;
                      let allSlotsFull = true;
                      let hasRegisterableSlot = false;

                      if (session.type === 'livestream' && session.timeSlots) {
                        hasRegisteredSlot = session.timeSlots.some(slot => slot.registrationStatus === 'registered');
                        allSlotsFull = session.timeSlots.every(slot => slot.registrationStatus === 'full');
                        hasRegisterableSlot = session.timeSlots.some(slot => slot.registrationStatus === 'register');
                      }
