...
import { allLessons } from "@/data/courseData"; // Import allLessons from central data file

const LessonDetailPage = () => {
  const { lessonId } = useParams<{ lessonId: string }>();

  const currentLessonData = allLessons.find(lesson => lesson.sessionId === lessonId);
    ...
    <div className="min-h-screen bg-gray-50 flex flex-col">
      ...
      <main className="flex-grow container mx-auto px-4 py-8 grid grid-cols-1 lg:grid-cols-2 gap-8">
        <div className="lg:col-span-1">
          ...
          <LessonHero
            lessonTitle={currentLessonData.title}
            teacherName={currentLessonData.teacher || "N/A"}
            viewsRemaining={currentLessonData.views || 0}
          />
        </div>
        <div className="lg:col-span-1">
